...
data1 <- ames %>% slice_head(n = 3) %>% select(Lot_Area, Sale_Price) %>% mutate(across(everything(), as.numeric))
data2 <- ames %>% slice_head(n = 3) %>% select(Lot_Area, Bsmt_Unf_SF, Sale_Price) %>% mutate(across(everything(), as.numeric))
min_intercept <- 0
max_intercept <- 250
```
...
tableOutput("error_intercept_only")
```
...
output$error_intercept_only <- renderTable({
  intercept <- input$intercept
  data1 %>% 
    mutate(prediction = intercept, residual = Sale_Price - prediction) %>% 
    summarize(
